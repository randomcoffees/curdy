...
module.exports.generateController = (model, modelName, templates, templateOverrides = { create: {}, delete: {}, show: {}, showAll: {}, update: {} }) => {
  // showAll.find
  if (!get(templateOverrides, 'showAll.find')) {
    set(templateOverrides, 'showAll.find', {});
  }

  // delete.render
  if (!get(templateOverrides, 'delete.render')) {
    set(templateOverrides, 'delete.render', {
      success: () => {
        return true;
      }
    });
    ...
    methodNames.forEach((methodName) => {
      ...
      methodStages.forEach((stageName) => {
        controllerMethods[methodName][stageName] = generateMethod(
          ...
          modelName,
          templates[stageName],
          templateOverrides[methodName] ? templateOverrides[methodName][stageName] : null
        );
