...
const generateMethod = (method, model, modelPath, template, templateOverride) => {
  return method(
    model,
    modelPath,
...
module.exports.generateController = (model, templates, opts = { templateOverrides: { create: {}, delete: {}, patch: {}, show: {}, showAll: {}, update: {} } }) => {
  // req.model.path
  if (!get(opts, 'req.model.path')) {
    set(opts, 'req.model.path', `curdy.${camelCase(model.modelName)}`);
  }

  // templateOverrides.showAll.find
  if (!get(opts, 'templateOverrides.showAll.find')) {
    set(opts, 'templateOverrides.showAll.find', {});
  }

  // templateOverrides.delete.render
  ...
  const methodNames = ['create', 'delete', 'patch', 'show', 'showAll', 'update'];
...
module.exports.generateRoutes = (Controller, isRouting = { create: true, delete: true, patch: true, show: true, showAll: true, update: true }, idString = ':_id', routerOpts = {}) => {
  ...
  if (isRouting.patch) {
    router.patch(
      `/${idString}`,
      Controller.patch.pre.find,
      Controller.patch.find,
      Controller.patch.post.find,

      Controller.patch.pre.operation,
      Controller.patch.operation,
      Controller.patch.post.operation,

      Controller.patch.pre.render,
      Controller.patch.render,
      Controller.patch.post.render
    );
  }
  if (isRouting.show) {
    router.get(
