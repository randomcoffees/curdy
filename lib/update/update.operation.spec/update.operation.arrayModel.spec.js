...
describe('update.operation.arrayModel.spec', () => {
  beforeEach(async () => {
    ...
    this.arrayModelObject = Object.assign(this.arrayModel.toJSON(), {
      string: null,
      boolean: null,
      // list: [],
      number: null
  ...
  describe('list path', () => {
    ...
    for(let i = 0; i < 1; i++) {
      describe(`list.${i} path`, () => {
        it.only('must update a simple model via body', async () => {
          const req = {
            arrayModel: this.arrayModel,
            body: {
          ...
          this.arrayModelObject.updatedAt = arrayModel.updatedAt;
          this.arrayModelObject.__v = arrayModel.__v;
          console.log(JSON.stringify(arrayModel.toJSON(), null, 2));
          expect(arrayModel.toJSON()).to.deep.equal(this.arrayModelObject);
          expectEqual(arrayModel.toJSON(), this.arrayModelObject);
        });
      });
